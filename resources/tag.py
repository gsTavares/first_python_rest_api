...
class LinkTagsToItem(MethodView):
    ...
    def delete(self, item_id, tag_id):
        ...
        try:
            ...
            abort(500, "An error ocurred while removing the tag.")
...
class Tag(MethodView):
    ...
    def delete(self, tag_id):
        ...
        if not tag.items: # checks if the tag is linked to any item
            db.session.delete(tag)
            db.session.commit()
            return {"message": "Tag deleted."}

        abort(400, message="Could not delete tag. Make sure tag is not associated with any item, them try again.")